...
usage() {
  cat <<'EOF'
Usage: bash scripts/create-clean-env.sh <environment-name>

Creates a new Geneva Event Companion azd environment in East US 2 and configures
the required Foundry Bicep inputs. The environment name must follow:
geneva-companion-<suffix>-eus2
EOF
}
...
environment_name="${1:-}"
if [[ ! "$environment_name" =~ ^geneva-companion-[a-z0-9-]+-eus2$ ]]; then
  usage >&2
  exit 2
fi
