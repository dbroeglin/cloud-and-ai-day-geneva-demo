...
usage() {
  cat <<'EOF'
Usage: bash scripts/create-clean-env.sh geneva

Creates the Geneva Event Companion azd environment in France Central and configures
...
environment_name="${1:-}"
if [[ "$environment_name" != "geneva" ]]; then
  usage >&2
  exit 2
fi
...
AZURE_DEV_USER_AGENT=microsoft_foundry_skill azd env config set infra.parameters.location francecentral
AZURE_DEV_USER_AGENT=microsoft_foundry_skill azd env config set \
